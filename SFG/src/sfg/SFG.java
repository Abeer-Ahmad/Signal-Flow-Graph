...
public class SFG implements ISFG {
	...
	private ArrayList<ArrayList<Integer>> loopGains;
	...
	SFG(int size) {
		...
		loopGains = new ArrayList<ArrayList<Integer>>();
	}
	...
	public ArrayList<ArrayList<String>> loops() {
		...
		for (int i = 0; i < nodes.length; i++)
			loops(nodes[i], nodes[i], false, visited, new ArrayList<String>(), new ArrayList<Integer>());
		return loops;
	}
	...
	private void loops(Node curNode, Node destination, boolean visit, boolean visited[],
			ArrayList<String> currentLoop, ArrayList<Integer> currentGain) {
		int nodeIndex = (int) indices.get(curNode.name());
	...
	private void addLoop(ArrayList<String> currentLoop, ArrayList<Integer> currentGain) {
		int counter;
		boolean exist = false;
