...
public class Main {
	...
	public static void main(String[] args) {
	    ...
	    System.out.println("Following are all different paths from y1 to y7:");
	    ArrayList<ArrayList<String>> paths = g.forwardPaths(y1, y7);
	    ...
	    System.out.println("Following are all different gains from y1 to y7:");
	    ArrayList<ArrayList<Integer>> gains = g.forwardGains();
	    for (int i = 0; i < gains.size(); i++) {
			for (int j = 0; j < gains.get(i).size(); j++)
				System.out.print(gains.get(i).get(j) + " ");
			System.out.println();
		}
	}
